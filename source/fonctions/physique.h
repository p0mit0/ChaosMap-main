#pragma once
#include <iostream>
#include <vector>
#include <cmath>
#include <fstream>

struct vec2 // structure vecteur dimension 2
{
    double x;
    double y;
};

struct vec3
{
    double x;
    double y;
    double k;
};
double norme(vec2 v) // fonction calcule norme
{
    double x = v.x;
    double y = v.y;
    // instructions
    return sqrt(x * x + y * y);
};

// type pôle (pas nécessaire mais pratique)
struct Pole
{
    vec2 pos;
    double k;
};
// tableau de pôles
std::vector<Pole> liste_poles =
    {

        {{35, 120}, 85},
        {{160, 45}, 110},
        {{95, 180}, 60},
        {{110, 25}, 90},
        {{50, 150}, 75},
        {{140, 130}, 105},
        {{70, 70}, 95},
        {{125, 165}, 80},
        {{20, 80}, 120},
        {{175, 100}, 70},
        {{85, 40}, 100},
        {{150, 85}, 65},
        {{60, 110}, 115},
        {{105, 145}, 55},
        {{45, 35}, 105},
        {{130, 50}, 85},
        {{80, 160}, 90},
        {{165, 155}, 75},
        {{25, 170}, 100},
        {{180, 20}, 80}};

const int RES = 201;                      // taille de l'espace, malheureusement je n'ai pas eu le temps de modifier afin que cela augmente la résolution plutot que
                                          // agrandir l'espace
vec2 Mat_space[RES][RES];                 // création de la matrice de vecteurs positions
vec2 Mat_space_var[RES][RES];             // matrice +delta donc des positions décalées
vec2 Mat_space_euler[RES][RES];           // matrice positions de euler
vec2 Mat_space_var_euler[RES][RES];       // matrice positions +delta de euler
vec2 Mat_space_velo_verlet[RES][RES];     // matrice de velocity verlet
vec2 Mat_space_var_velo_verlet[RES][RES]; // matrice +delta de velocity verlet
vec2 Mat_space_yoshida[RES][RES];         // matrice de yoshida
vec2 Mat_space_var_yoshida[RES][RES];     // matrice positions +delta de yoshida
vec2 Mat_space_rk4[RES][RES];             // matrice position de rk4
vec2 Mat_space_var_rk4[RES][RES];         // matrice positions +delta de rk4
double Mat_chaos_euler[RES][RES];         // création de la matrice contenant le chaos (méthode euler)
// double Mat_chaos_norm_euler[RES][RES];       // matrice contenant le chaos normée (méthode euler)
double Mat_chaos_velo_verlet[RES][RES]; // création de la matrice contenant le chaos (méthode verlet)
// double Mat_chaos_norm_velo_verlet[RES][RES]; // matrice contenant le chaos normée (méthode verlet)
double Mat_chaos_yoshida[RES][RES]; // création de la matrice contenant le chaos (méthode euler)
// double Mat_chaos_norm_yoshida[RES][RES];// création de la matrice contenant le chaos (méthode euler)
double Mat_chaos_rk4[RES][RES]; // création de la matrice contenant le chaos (méthode euler)
// double Mat_chaos_norm_rk4[RES][RES];// création de la matrice contenant le chaos (méthode euler)
double dt = 1e-1;              // initialisation, constantes
double t_f = 20;               // temps final
double m = 1;                  // masse de l'objet
double v_0_x = 3;              // vitesse initiale selon x
double v_0_y = 3;              // vitesse initiale selon y
double delta = 1e-3;           // décalage entre position initiale et finale
double r_capture = 2;          // rayon a partir du quel on considère une particule comme capturée par le pôle
vec2 dist(vec2 pos, Pole pole) //  fonction calcul distance
{
    return {(double)(pos.x - pole.pos.x), (double)(pos.y - pole.pos.y)};
}

double r_cube(double r)
{
    return r * r * r;
}

// fonction acceleration pour pole dans liste_pole:

vec2 accel(double pos_x, double pos_y)
{
    double ax = 0;
    double ay = 0;

    for (size_t p = 0; p < liste_poles.size(); p++)
    {
        vec2 d = dist({pos_x, pos_y}, liste_poles[p]);
        ax += (liste_poles[p].k / (r_cube(norme(d))) * (liste_poles[p].pos.x - pos_x)) / m;
        ay += (liste_poles[p].k / (r_cube(norme(d))) * (liste_poles[p].pos.y - pos_y)) / m;
    }
    return {ax, ay};
}
bool condition_capture(double &x, double &y) // fonction condition de capture
{
    for (size_t i = 0; i < liste_poles.size(); i++)
    {
        vec2 d = dist({x, y}, liste_poles[i]);

        if (norme(d) <= r_capture)
        {
            x = liste_poles[i].pos.x;
            y = liste_poles[i].pos.y;
            return true;
        }
    }
    return false;
}
